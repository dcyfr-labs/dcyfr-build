 ...
 *
 * dcyfr-build's instance was hover-only: `bg-muted hover:bg-primary` with a
 * flat `text-foreground`, so the label disappeared on hover in dark.
 * NOTE the limitation — this gate walks resting computed style, so it would
 * NOT have caught that one. It is ported for the resting-state coverage;
 * the hover pair was found by the fleet grep and fixed by hand.
 *
